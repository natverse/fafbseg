...
test_that("decode scene works", {
  ...
  expect_is(sc <- ngl_decode_scene(url), "ngscene")
  expect_known_value(sc, file = "testdata/ngscene.rds")
  expect_equal(ngl_encode_url(sc), url)
  # expect_is(json <- ngl_decode_scene(url, return.json = TRUE), "character")
  # writeLines(json, con='tests/testthat/testdata/ngurl.json')
  expect_equal(ngl_decode_scene(url, return.json = TRUE),
